...
class ENV:
    def __init__(self,graph):
        ...
        if i == graph._data.shape[0] - 1: i += 1
        ...
        if i == self.edge_type.shape[1] - 1: i += 1
    ...
    def get_state(self):
        ...
        #return ret
        #print(self.edge_index)
        return self.data[:self.item_count_real],self.edge_index,self.edge_type
    def step(self, action):
        ...
        if collisions >= collisions_orig: reward = -collisions * 2
    ...
    def getboxcollision(self):
        ...
        for i in range(len(mgrs)):
            for j in range(i+1, len(mgrs)):
                ...
                if is_collision:
                    edge_idx_tmp.append([i,j])
                    edge_idx_tmp.append([j,i])
