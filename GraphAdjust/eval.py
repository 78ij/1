...
policy_net.load_state_dict(torch.load('./net_imitationab_all.pth'))
    ...
    #     #  print(action)
     #   env_list[i].set_state(state)
      ...
      #  print(reward)
        ...
        all_data, all_edge,all_type,data,edge_index,edge_type,subbox_lengths,wallcountour = env_list[i].get_state()
    #print(edge_index)
        length = [len(subbox_lengths)]
        all_data = torch.tensor(all_data.astype("float32"),device=device)
        all_edge = torch.tensor(all_edge.astype("long"),device=device)
