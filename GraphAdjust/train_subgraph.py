  ...
  #  print(dones)
   ...
   # print(non_final_next_states)
    ...
    if(len(non_final_next_states) != 0):
        ...
        next_state_tmp,_ = target_net(all_edge_next.reshape(1,-1,2), all_type_next.reshape(1,-1,3), next_state_input.reshape(1,-1,11), next_edge_index_input.long().reshape(1,-1,2), next_edge_type_input.reshape(1,-1,3),next_lengths,next_subbox_lengths,next_wall_countours)
        next_state_tmp_value,_ = policy_net(all_edge_next.reshape(1,-1,2), all_type_next.reshape(1,-1,3),next_state_input.reshape(1,-1,11), next_edge_index_input.long().reshape(1,-1,2), next_edge_type_input.reshape(1,-1,3),next_lengths,next_subbox_lengths,next_wall_countours)
    ...
    state_action_values,loss_diff = policy_net(all_edge.reshape(1,-1,2), all_type.reshape(1,-1,3), state_input.reshape(1,-1,11), edge_index_input.long().reshape(1,-1,2), edge_type_input.reshape(1,-1,3),lengths,subbox_lengths,wall_countours)
...
with Pool(processes=10) as pool:
    while True:
        #pool.map(train_iteration, range(10))
        #for i in range(10):
        train_iteration(0)
