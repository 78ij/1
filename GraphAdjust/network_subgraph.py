...
class DQNetwork_simple(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.gnnencoder_upper = GNNEncoder(256+4+7,256, 512,'avg','avg',2,3)
       # self.wall_mlp = nn.Linear(1024 * 3, 512)
        ...
        self.mlp2_s = nn.Linear(int(hidden_size / 2), 1) 

        #self.pointnet = PointNetfeat(global_feat=True)

    def forward(self, all_data,all_edge,all_type, data_input,edge_index,edge_type,lengths, subbox_lengths,wall_countour):
        #print(state_input)
      ...
      #  wall_countour_out = torch.cat(wall_countour_out,dim=0)
        ...
        graph_feature_out = torch.cat([graph_feature_out,all_data.reshape(-1,7),wall_countour],dim=1).reshape(1,-1,256+4+7)
       ...
       # print(state_input.shape)
        ...
        out_action = torch.tanh(self.mlp2(torch.sigmoid(self.mlpm(torch.sigmoid(state_input))))) * 100
        ...
        out_value = torch.tanh(self.mlp2_v(torch.sigmoid(self.mlpm_v(torch.sigmoid(value_state_input))))) * 100
        out_s = torch.tanh(self.mlp2_v(torch.sigmoid(self.mlpm_v(torch.sigmoid(value_state_input))))) * 100
       # loss_diff = torch.mean(torch.exp(-torch.abs(node_from - node_to)))
        loss_diff = 0
        #print(out.shape)
        #print(edge_index)
        out_action = list(out_action.split(lengths,dim=0))
        feasible = (wall_countour < 0.0001) * -1000000
        feasible = feasible.split(lengths,dim=0)
        #print(out)
        out_final = torch.ones((len(out_action),84),device='cuda:0') * -10000000
        for i in range(len(out_action)):
            out_final[i,0:4*out_action[i].shape[0]] = out_action[i].flatten() - torch.mean(out_action[i].flatten()) + out_value[i] + feasible[i].flatten()
            out_final[i,4*out_action[i].shape[0]:4*out_action[i].shape[0]+4] = out_s[i].repeat(4).flatten()
