...
memory = []
for i in range(5):
    policy_net.append(network_subgraph.DQNetwork_simple(512).to(device))
    target_net.append(network_subgraph.DQNetwork_simple(512).to(device))
    target_net[i].load_state_dict(policy_net[i].state_dict())
    target_net[i].eval()
    optimizer.append(optim.Adam(policy_net[i].parameters(),lr=LR, weight_decay=0.00001))

    #optimizer = optim.RMSprop(policy_net.parameters(),lr=0.00001)
    scheduler.append(optim.lr_scheduler.StepLR(optimizer[i], step_size=10000, gamma=0.95))
    # memory = ReplayBuffer(
    #             action_size_per_item * item_max_size, BUFFER_SIZE, BATCH_SIZE, EXPERIENCES_PER_SAMPLING, 12254151, True)
    memory.append(PrioritizedReplayBuffer(30000, alpha=0.4))
...
def select_action(state, size,idx):
    global steps_done
    ...
    all_edge,all_type,data,edge_index,edge_type,subbox_lengths,wall_countour = state
    #print(edge_index)
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            out,_ = policy_net[idx](all_edge.T.reshape(1,-1,2), all_type.T.reshape(1,-1,3),data.reshape(1,-1,11), edge_index.long().T.reshape(1,-1,2), edge_type.T.reshape(1,-1,3),length,subbox_lengths,wall_countour)
            print(out)
            #print(out.max(0))
...
def merge_graph(state_batch):
    ...
    for s in state_batch:
        all_edge_idx_tmp, all_edge_type_tmp, data_tmp,edge_index_tmp,edge_type_tmp,subbox_lengths_tmp,wall_countour_tmp = s
    ...
    return np.concatenate(all_edge_idx,axis=0),np.concatenate(all_edge_type,axis=0), np.concatenate(data_ret,axis=0).reshape(1,-1,11),np.concatenate(edge_index_ret,axis=0).reshape(1,-1,2),np.concatenate(edge_type_ret,axis=0).reshape(1,-1,3),lengths,subbox_lengths,np.concatenate(wall_countours,axis=0)

def optimize_model(idx,x=False):
    print('aaaaaaaaaa')
    global episode_now
    episode_now += 1
    ...
    states, actions,rewards,next_states,dones,is_im,weights,idxs = memory[idx].sample(BATCH_SIZE, beta=beta_schedule.value(episode_now))
    #print(idx)
 ...
 #   print('final ' + str(final_mask))
  ...
  #  print(dones)
    ...
    all_edge, all_type , state_input, edge_index_input, edge_type_input,lengths,subbox_lengths,wall_countours= merge_graph(states)
   ...
   # print(non_final_next_states)
    ...
    if(len(non_final_next_states) != 0):
        all_edge_next, all_type_next, next_state_input, next_edge_index_input, next_edge_type_input,next_lengths,next_subbox_lengths,next_wall_countours = merge_graph(non_final_next_states)
        ...
        next_state_tmp,_ = target_net[idx](all_edge_next.reshape(1,-1,2), all_type_next.reshape(1,-1,3), next_state_input.reshape(1,-1,11), next_edge_index_input.long().reshape(1,-1,2), next_edge_type_input.reshape(1,-1,3),next_lengths,next_subbox_lengths,next_wall_countours)
        next_state_tmp_value,_ = policy_net[idx](all_edge_next.reshape(1,-1,2), all_type_next.reshape(1,-1,3),next_state_input.reshape(1,-1,11), next_edge_index_input.long().reshape(1,-1,2), next_edge_type_input.reshape(1,-1,3),next_lengths,next_subbox_lengths,next_wall_countours)
    ...
    state_action_values,loss_diff = policy_net[idx](all_edge.reshape(1,-1,2), all_type.reshape(1,-1,3), state_input.reshape(1,-1,11), edge_index_input.long().reshape(1,-1,2), edge_type_input.reshape(1,-1,3),lengths,subbox_lengths,wall_countours)
 ...
 #   print(actions)
  #  print(state_action_values)
   ...
   # print(actions.shape)
    ...
    all_edge_dloss, all_type_dloss , state_input_dloss, edge_index_input_dloss, edge_type_input_dloss,lengths_dloss,subbox_lengths_dloss,wall_countours_dloss= merge_graph(next_states)

    all_edge_dloss = torch.tensor(all_edge_dloss.astype("long"),device=device)
    all_type_dloss = torch.tensor(all_type_dloss.astype("float32"),device=device)
    state_input_dloss = torch.tensor(state_input_dloss.astype("float32"),device=device)
    edge_index_input_dloss = torch.tensor(edge_index_input_dloss.astype("long"),device=device)
    edge_type_input_dloss = torch.tensor(edge_type_input_dloss.astype("float32"),device=device)
    wall_countours_dloss = torch.tensor(wall_countours_dloss.astype("float32"),device=device)
  #  done_net_out = D_net(all_edge_dloss.reshape(1,-1,2), all_type_dloss.reshape(1,-1,3), state_input_dloss.reshape(1,-1,11), edge_index_input_dloss.long().reshape(1,-1,2), edge_type_input_dloss.reshape(1,-1,3),lengths_dloss,subbox_lengths_dloss,wall_countours_dloss)
 ...
 #   print(state_action_values_true)
  ...
  #  print(idxs)
    ...
    memory[idx].update_priorities(idxs, abs_errors.detach().cpu().numpy().tolist())  
    # print('abs_errors ') 
    # print(abs_errors)
    # print('state_action_values ') 
   ...
   # loss = F.mse_loss( state_action_values.flatten(), expected_state_action_values.flatten())
    ...
    #print(expected_state_action_values)
    #print(rewards)
    lr = optimizer[idx].param_groups[0]['lr']
    # print(
...
#i_episode = 0
for xx in range(len(memory)):
    memory[xx].start_real_sample()
#while(True):
...
def train_iteration(i_episode):
 #  sys.stdout.flush()
    ...
    idx_step = 0
    print('1')
    for round in range(800):
        env_list[i_episode].reset()
        for t in range(30):
            # if idx_step % TARGET_UPDATE == 0:
            #     target_net.load_state_dict(policy_net.state_dict())
            # if idx_step % 5000 == 0:
            #     torch.save(policy_net.state_dict(), './net.pth')
            #     torch.save(D_net.state_dict(), './net_d.pth')
        #  idx_step = idx_step + 1
            #print(t)y
            # Select and perform an action
            state = env_list[i_episode].get_state()
            action = select_action(state,env_list[i_episode].item_count_real,i_episode)
            reward, done = env_list[i_episode].step(action.item())
            #print(reward)
            #reward = torch.tensor([reward], device=device)

            # Observe new state
            #if not done:
            next_state = env_list[i_episode].get_state()
            #  else:
            #     next_state = None
            ...
            memory[i_episode].add(state, action.item(), reward, next_state, done, False)
           # if idx_step % TARGET_UPDATE == 0:
           #     target_net[idx].load_state_dict(policy_net.state_dict())
            if idx_step % 500 == 0:
                torch.save(policy_net[i_episode].state_dict(), './models/net' + str(i_episode) + '.pth')
              #  torch.save(D_net.state_dict(), './models/net_d.pth')
            #lock.release()
...
with Pool(processes=5) as pool:
    #while True:
    #pool.map(train_iteration, range(5))
        #for i in range(10):
    train_iteration(1)
