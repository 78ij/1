...
class MoveableEncoder(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size, orient = False):
        ...
        self.mlp_skip = nn.Linear(8, feature_size)
...
class BoxEncoder1(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size, orient = False):
        ...
        self.mlp1 = nn.Sequential(nn.Linear(7, hidden_size), nn.LeakyReLU(0.1), nn.Linear(hidden_size, hidden_size))
        self.mlp2 = nn.Linear(hidden_size, feature_size)
...
class BoxEncoder(nn.Module):
    ...
    def __init__(self, feature_size):
        ...
        self.encoder = nn.Linear(7, feature_size)
...
class BoxDecoder1(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size):
        ...
        self.angle = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.LeakyReLU(0.1), nn.Linear(hidden_size, 1))
        self.center_skip = nn.Linear(feature_size, 3)
        self.size_skip = nn.Linear(feature_size, 3)
        self.angle_skip = nn.Linear(feature_size, 1)


    def forward(self, parent_feature):
        feat = torch.nn.functional.leaky_relu(self.mlp(parent_feature), 0.1)
        center = torch.tanh(self.center(feat) + self.center_skip(parent_feature)) * 10
        # size = torch.sigmoid(self.size(feat)) * 22
        # size = torch.abs(self.size(feat) + self.size_skip(parent_feature))
        size = torch.sigmoid(self.size_skip(parent_feature)+self.size(feat)) * 10
        ...
        rot = (self.angle_skip(parent_feature) + self.angle(feat))
        vector = torch.cat([center, size, rot], dim=1)
        return vector


class MoveableDecoder(nn.Module):

    def __init__(self, feature_size, hidden_size):
        super(MoveableDecoder, self).__init__()
        self.mlp = nn.Linear(feature_size, hidden_size)

        self.center = nn.Linear(hidden_size, 3)
        self.dir = nn.Linear(hidden_size, 3)
        self.limits = nn.Linear(hidden_size, 2)

    def forward(self, parent_feature):
        feat = torch.relu(self.mlp(parent_feature))

        center = torch.tanh(self.center(feat)) * 10
        dir_pred = torch.tanh(self.dir(feat))
        #quat_bias = feat.new_tensor([[1.0, 0.0, 0.0, 0.0]])
        limits = self.limits(feat)#.add(quat_bias)
        #quat = quat / (1e-12 + quat.pow(2).sum(dim=1).unsqueeze(dim=1).sqrt())
        vector = torch.cat([center, dir_pred, limits], dim=1)
        return vector
...
class BoxDecoder(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size):
        ...
        self.quat = nn.Linear(hidden_size, 1)

    def forward(self, parent_feature):
        feat = torch.relu(self.mlp(parent_feature))
        center = torch.tanh(self.center(feat)) * 10
        size = torch.sigmoid(self.size(feat)) * 10
        #quat_bias = feat.new_tensor([[1.0, 0.0, 0.0, 0.0]])
...
class GNNEncoderStructureNet(nn.Module):
    ...
    def __init__(self, node_feat_size, hidden_size, node_symmetric_type, \
        ...
        self.child_op = nn.Linear(node_feat_size * 2+ self.num_sem + 128, hidden_size)
        ...
        self.skip_op_object = nn.Linear(node_feat_size * 2 + self.num_sem + 128, node_feat_size)
    ...
    """
    def forward(self, child_feats, child_exists, edge_indices,edge_type_onehot):
...
class GNNDecoderStructureNet(nn.Module):
    ...
    def __init__(self, node_feat_size, hidden_size, max_child_num, \
        ...
        self.mlp_rootcode = nn.Linear(node_feat_size, 128)
        self.moveabledecoder = MoveableDecoder(node_feat_size,hidden_size)
    ...
    def graph_recon_loss(self, node_latent, gt_data, gt_edge_index,gt_edge_type):
        gt_data = gt_data.reshape(1,-1,14 +128 + 8)
        gt_edge_index = gt_edge_index.reshape(1,-1,2)
        #print(gt_edge_index)

        child_feats, child_sem_logits, child_exists_logits, edge_exists_logits = self.forward(node_latent)
        ...
        root_code_pred = self.mlp_rootcode(child_feats.view(-1, feature_len))
        root_code_pred = root_code_pred.view(1, self.max_child_num, 128)

        moveable_pred = self.moveabledecoder(child_feats.view(-1, feature_len))
        moveable_pred = moveable_pred.view(1, self.max_child_num, 8)

        num_child_parts = child_pred_boxes.size(0)
        #print(child_pred_boxes)
        # perform hungarian matching between pred boxes and gt boxes
        with torch.no_grad():
            child_gt_boxes = gt_data[0, :,:7]
            ...
            dist_mat = self.boxLossEstimator(child_gt_boxes_tiled.view(-1, 7), child_pred_boxes_tiled.view(-1, 7)).view(-1, num_gt, num_child_parts)
            ...
            edge_type_list_gt = gt_edge_type
           # print(edge_indices_gt.shape)
            ...
            for i in range(edge_indices_gt.shape[1]//2):
               ...
               # print(edge_exists_logits)
                ...
                if edge_type_list_gt[0, i,0] == 1: # SAME
                    same_from.append(edge_from_idx)
                    same_to.append(edge_to_idx)

                if edge_type_list_gt[0, i,0] == 2: # SAME
                    moveable_from.append(edge_from_idx)
                    moveable_to.append(edge_to_idx)
        ...
        # and root codes to gt
        box_loss = 0
        anchor_loss = 0
        moveable_recon_loss = 0
        for i in range(len(matched_gt_idx)):
            box_loss += self.boxLossEstimator(gt_data[0,matched_gt_idx[i],:7].reshape(1,-1),child_pred_boxes[matched_pred_idx[i],:].reshape(1,-1))
            #box_loss += self.mseLoss(gt_data[0,matched_gt_idx[i],:7].reshape(1,-1),child_pred_boxes[matched_pred_idx[i],:].reshape(1,-1)).sum()
            #anchor_loss = self.anchorLossEstimator(gt_data[0,matched_gt_idx[i],:7].reshape(1,-1),child_pred_boxes[matched_pred_idx[i],:].reshape(1,-1))
            anchor_loss = self.mseLoss(gt_data[0,matched_gt_idx[i],6:7].reshape(1,-1),child_pred_boxes[matched_pred_idx[i],6:7].reshape(1,-1)).sum() * 10
            rootcode_loss = self.mseLoss(gt_data[0,matched_gt_idx[i],14:142].reshape(1,-1),root_code_pred[0,matched_pred_idx[i],:].reshape(1,-1)).sum()

            moveable_recon_loss += self.mseLoss(gt_data[0,matched_gt_idx[i],142:].reshape(1,-1),moveable_pred[0,matched_pred_idx[i],:].reshape(1,-1)).sum()

           # print(gt_data[0,matched_gt_idx[i],142:])
           # print(moveable_pred[0,matched_pred_idx[i],:])
           # print(moveable_recon_loss)
        ...
        for i in range(len(matched_gt_idx)):
            gt_label_tmp = gt_data[0,matched_gt_idx[i],7:14]
       ...
       # print(child_sem_gt_labels)
        ...
        same_loss = 0
        if len(same_from) > 0:
            same_from_th = torch.tensor(same_from, dtype=torch.long, device='cuda:0')
            obb_from = child_pred_boxes[same_from_th, :]
            same_to_th = torch.tensor(same_to, dtype=torch.long, device='cuda:0')
            obb_to = child_pred_boxes[same_to_th, :]
            same_loss = self.mseLoss(obb_from[3:7].reshape(1,-1),
            obb_to[3:7].reshape(1,-1)).sum()
           
            same_loss += self.mseLoss(root_code_pred[0,same_from_th,:].reshape(1,-1),
            root_code_pred[0,same_to_th,:].reshape(1,-1)).sum()
        
        moveable_loss = 0
        if len(moveable_from) > 0:
            moveable_from_th = torch.tensor(moveable_from, dtype=torch.long, device='cuda:0')
            obb_from = child_pred_boxes[moveable_from_th, :]
            moveable_to_th = torch.tensor(moveable_to, dtype=torch.long, device='cuda:0')
            obb_to = child_pred_boxes[moveable_to_th, :]
            
            moveable_loss = self.mseLoss(obb_from[0:3].reshape(1,-1),
            obb_to[0:3].reshape(1,-1)).sum()
        ...
        return {'box': box_loss + unused_box_loss , 'anchor': anchor_loss, 
                 'exists': child_exists_loss, 'semantic': semantic_loss,
                 'edge_exists': edge_exists_loss, 'rootcode': rootcode_loss, 'same' : same_loss, 'moveable': moveable_loss,'moveable_recon':moveable_recon_loss}