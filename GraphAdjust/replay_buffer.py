...
class ReplayBuffer:
       ...
       # print(tds)
        ...
        for td, index in zip(tds, indices):
            ...
            if self.compute_weights:
                updated_weight = ((N * updated_priority+ 0.0000001)**(-self.beta))/self.weights_max
    ...
    def update_parameters(self):
        ...
        for element in self.memory_data.values():
            ...
            if self.compute_weights:
                weight = ((N *  element.probability + 0.0000001)**(-self.beta))/self.weights_max
    ...
    def sample(self):
        ...
        actions = torch.cat([e.action for e in experiences if e is not None],0)
