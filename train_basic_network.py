...
from config import add_train_ae_args
import utils
...
def train(conf):
    ...
    print(f'Room Category: {conf.category}')
    flog.write(f'Room Category: {conf.category}\n')
    ...
    train_dataset = SceneGraphDataSet(conf.data_path, 'MasterBedroom',valid=False)
    valdt_dataset = SceneGraphDataSet(conf.data_path, 'MasterBedroom',valid=True)
    ...
    if not conf.no_console_log:
        header = '     Time    Epoch     Dataset    Iteration    Progress(%)       LR       BoxLoss   StructLoss   EdgeExists  RootCodeLoss   SameLoss  MoveableLoss  MoveableReconLoss   AnchorLoss  TotalLoss'
   ...
   # print(batch)
    losses = {
       ...
       # 'adj': torch.zeros(1, device=device)},
        'rootcode': torch.zeros(1, device=device),
        'same': torch.zeros(1, device=device),
        'moveable': torch.zeros(1, device=device),
        'moveable_recon': torch.zeros(1, device=device)}
...
#losses[loss_name] = losses[loss_name] / len(objects)
    ...
    losses['rootcode'] *= conf.loss_weight_rootcode
    #losses['kldiv'] *= conf.loss_weight_kldiv
    # losses['sym'] *= conf.loss_weight_sym
    # losses['adj'] *= conf.loss_weight_adj
    losses['same'] *= conf.loss_weight_same
    losses['moveable'] *= conf.loss_weight_moveable
    losses['moveable_recon'] *= conf.loss_weight_moveable_recon
    ...
    with torch.no_grad():
        ...
        if log_console:
            print(
                ...
                f'''{(losses['exists']+losses['semantic']).item():>11.2f} '''
                f'''{losses['edge_exists'].item():>11.2f} '''
                f'''{losses['rootcode'].item():>10.2f} '''
                #f'''{losses['kldiv'].item():>10.2f} '''
                #f'''{losses['sym'].item():>10.2f} '''
                #f'''{losses['adj'].item():>10.2f} '''
                f'''{losses['same'].item():>10.2f} '''
                f'''{losses['moveable'].item():>10.2f} '''
                f'''{losses['moveable_recon'].item():>10.2f} '''
        ...
        if log_tb and tb_writer is not None:
            ...
            tb_writer.add_scalar('root_code_loss', losses['rootcode'].item(), step)
            tb_writer.add_scalar('same_struc_loss', losses['same'].item(), step)
            tb_writer.add_scalar('moveable_struc_loss', losses['moveable'].item(), step)
            tb_writer.add_scalar('moveable_recon_loss', losses['moveable_recon'].item(), step)
...
if __name__ == '__main__':
    ...
    parser = add_train_ae_args(parser)
