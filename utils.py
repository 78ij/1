...
def transform_pc1(pc, feat):
    num_point = pc.shape[0]
    center = feat[:3]
    shape = feat[3:6]
    #quat = torch.zeros(feat.shape[0],4,device='cuda:0')
    quat = quaternion_about_y1(feat[6])
    #for i in range(feat.shape[0]):
    #    quat[i,:] = quaternion_about_axis(feat[i, 6],[0,1,0])
    pc = qrot(quat.repeat(num_point, 1), pc)
    pc = pc + center.repeat(num_point, 1)
    return pc
...
def quaternion_about_y(angle):
    ...
    q = torch.zeros([angle.shape[0],4],device='cuda:0')
    q[:,1:] = torch.tensor([0,1,0],device='cuda:0').reshape(1,3).repeat(angle.shape[0],1)
    #q = torch.tensor([0.0, axis[0], axis[1], axis[2]],device='cuda:0')
    #qlen = torch.norm(q)
    #if qlen > _EPS:
    q *= torch.sin(angle / 2.0).reshape(-1,1).repeat(1,4)
    q[:,0] = torch.cos(angle / 2.0)
    return q

def quaternion_about_y1(angle):
    
    q = torch.zeros(4)
    q[1:] = torch.tensor([0,1,0])
    #q = torch.tensor([0.0, axis[0], axis[1], axis[2]],device='cuda:0')
    #qlen = torch.norm(q)
    #if qlen > _EPS:
    q *= torch.sin(angle / 2.0).repeat(4)
    q[0] = torch.cos(angle / 2.0)
    return q
...
def transform_pc_batch(pc, feat, anchor=False):
    ...
    #quat = torch.zeros(feat.shape[0],4,device='cuda:0')
    quat = quaternion_about_y(feat[:,6])
    #for i in range(feat.shape[0]):
    #    quat[i,:] = quaternion_about_axis(feat[i, 6],[0,1,0])
    quat = quat.unsqueeze(dim=1).repeat(1, num_point, 1)
    if not anchor:
        pc = pc * shape
...
def transform_pc_batch_angle(pc, feat, anchor=False):
    ...
    angle = feat[:, 6:7].unsqueeze(dim=1).repeat(1, num_point, 1)
    if not anchor:
        pc = pc * shape
    pc = qrot(quat.view(-1, 4), pc.view(-1, 3)).view(batch_size, num_point, 3)
    if not anchor:
