...
"""
    ...
    the leaf ndoe can predict the box parameter, with skip connenction operation
    log:
"""
...
from utils_sn import linear_assignment, load_pts, transform_pc_batch, get_surface_reweighting_batch, qrot
...
# borrow from the dgcnn network (pointnet)
...
class BoxEncoder(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size, orient = False):
        ...
        # if orient:
        #     self.encoder = nn.Linear(14, feature_size)
...
class PartEncoder1(nn.Module):

    def __init__(self, feat_len, probabilistic = False):
        super(PartEncoder1, self).__init__()
...
class NodeEncoder(nn.Module):
    ...
    def __init__(self, geo_feat_len, node_feat_len,orient = False):
        super(NodeEncoder, self).__init__()

        self.part_encoder = PartEncoder(geo_feat_len)
        self.box_encoder = BoxEncoder(geo_feat_len, orient = orient)

        # self.mlp1 = nn.Linear(4 + geo_feat_len, geo_feat_len)
        self.mlp2 = nn.Linear(2 * geo_feat_len + 4, node_feat_len)

    def forward(self, geo, boxf):
        ...
        box_feat = torch.cat([center, scale, self.box_encoder(boxf)], dim=1)
        ...
        all_feat = torch.nn.functional.leaky_relu(self.mlp2(all_feat), 0.1)
...
class SymmetricChildEncoder(nn.Module):
    ...
    def forward(self, child_feats, child_exists):
        ...
        skip_feats = torch.nn.functional.leaky_relu(self.skip_op_part(child_feats), 0.1)
        child_feats = torch.nn.functional.leaky_relu(self.child_op_part(child_feats), 0.1)
...
class RecursiveEncoder(nn.Module):
    ...
    def __init__(self, config, Tree, variational=False, probabilistic=True, orient = False):
        ...
        # self.node_encoder = NodeEncoder(config.geo_feat_size, config.feature_size, orient = orient)
    ...
    def encode_node(self, node):
        ...
        if node.is_leaf:
            ...
            # all_feature, geo_feature = self.node_encoder(node.geo, box_feature)
            # node.geo_feat = geo_feature
            all_feature = self.box_encoder(box_feature)
            return all_feature
        else:
...
class PartDecoder(nn.Module):
    ...
    def __init__(self, feat_len, num_point):
        ...
        self.mlp1 = nn.Linear(feat_len, feat_len*2)
        self.mlp2 = nn.Linear(feat_len*2, feat_len*4)
        self.mlp3 = nn.Linear(feat_len*4, feat_len*4)
        self.mlp4 = nn.Linear(feat_len*4, feat_len*4)
        self.mlp5 = nn.Linear(feat_len*4, num_point*3)

        self.bn1 = nn.BatchNorm1d(feat_len*2)
        self.bn2 = nn.BatchNorm1d(feat_len*4)
        self.bn3 = nn.BatchNorm1d(feat_len*4)
        self.bn4 = nn.BatchNorm1d(feat_len*4)

        self.chamferLoss = ChamferDistance()
        self.emdloss = earth_mover_distance
    ...
    def forward(self, net):
        ...
        net = torch.relu(self.bn3(self.mlp3(net)))
        net = torch.relu(self.bn4(self.mlp4(net)))
        net = self.mlp5(net).view(-1, self.num_point, 3)
    ...
    def loss(self, pred, gt):
        ...
        avg_loss = self.emdloss(pred, gt, transpose=False)/pred.size(1) *1000# pc1: B x N1 x 3, pc2: B x N2 x 3
        # print(avg_loss.mean())

        return avg_loss.mean()

class PartDecoder1(nn.Module):
...
class NodeDecoder(nn.Module):
    ...
    def __init__(self, geo_feat_len, node_feat_len, num_point, orient):
        super(NodeDecoder, self).__init__()

        self.mlp0 = nn.Linear(node_feat_len, node_feat_len*2)
        self.mlp1 = nn.Linear(node_feat_len, 3)
        self.mlp2 = nn.Linear(node_feat_len, 1)
        self.mlp3 = nn.Linear(node_feat_len, geo_feat_len)
        # self.mlp4 = nn.Linear(node_feat_len, geo_feat_len)

        self.part_decoder = PartDecoder(geo_feat_len, num_point)
        self.box_decoder = BoxDecoder(node_feat_len, geo_feat_len, orient = orient)

    def forward(self, net):
        all_feat = torch.nn.functional.leaky_relu(self.mlp0(net), 0.1)
        box_feat, geo_feat = torch.chunk(all_feat, 2, 1)
        geo_center = 11*torch.tanh(self.mlp1(box_feat))
        geo_scale = 22*torch.sigmoid(self.mlp2(box_feat))
        # box_feat = self.mlp4(net)
        box_quat = self.box_decoder(box_feat)

        geo_feat = self.mlp3(geo_feat)
        geo_local = self.part_decoder(geo_feat)

        return geo_local, geo_center, geo_scale, geo_feat, box_quat
...
class ConcatChildDecoder(nn.Module):
    ...
    def forward(self, parent_feature):
        ...
        # node features
        child_feats = self.mlp_child_part(parent_feature.view(-1, self.hidden_size))
...
class RecursiveDecoder(nn.Module):
    ...
    def __init__(self, config, Tree, orient = False, no_center = False):
        ...
        # self.node_decoder = NodeDecoder(config.feature_size, config.hidden_size, config.num_point, orient = self.orient)
    ...
    def boxLossEstimator(self, box_feature, gt_box_feature):
        ...
        loss = (loss1 + loss2) / 2
        # if len(box_feature) == 1:
        #     # loss1 = self.mseLoss(box_feature[:, :3], gt_box_feature[:, :3]).mean()
        #     # loss2 = self.L1Loss(box_feature[:, 3:6], gt_box_feature[:, 3:6]).mean() + torch.relu(gt_box_feature[:, 3:6] - box_feature[:, 3:6]).mean()
        #     # loss3 = self.mseLoss(box_feature[:, 6:], gt_box_feature[:, 6:]).mean()
        #     # loss = loss1 + loss2 + loss3
        #     loss = self.mseLoss(box_feature, gt_box_feature).mean()# + torch.relu(gt_box_feature[:, 3:6] - box_feature[:, 3:6]).mean()
        #     print(loss)
        # print(loss)
        return loss
    ...
    def node_recon_loss(self, node_latent, gt_node):
        ...
        if gt_node.is_leaf:
            ...
            # geo_loss = earth_mover_distance(geo_global, gt_geo, transpose = False)/geo_global.size(1)
            ...
            loss_acap = torch.zeros_like(box_loss)
            if self.orient:
            ...
            # print(box_loss)
            # print(box)
            # print(gt_node.get_box_quat())
            return {'box': box_loss + orient_loss, 'surf': geo_loss, 'center':center_loss + scale_loss, 'feat':latent_loss, 'leaf': is_leaf_loss,
                    'acap': loss_acap,
            ...
            # _, _, _, _, child_pred_boxes = self.node_decoder(child_feats.view(-1, feature_len))
            child_pred_boxes = child_pred_boxes[:, :10]
            ...
            for i in range(len(matched_gt_idx)):
                ...
                geo_loss = geo_loss + child_losses['surf']
            ...
            return {'box': box_loss + unused_box_loss, 'surf': geo_loss, 'center':center_loss, 'feat':latent_loss,
                    'acap': torch.zeros_like(box_loss),
