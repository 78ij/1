...
def draw_partnet_objects(objects, object_names=None, figsize=None, rep='boxes', \
        leafs_only=True, use_id_as_color=False, visu_edges=True, sem_colors_filename=None):
        ...
        for jj in range(len(part_boxes)):
            ...
            if rep == 'boxes':
                if part_boxes[jj] is not None:
                    draw_box(
                        ax=ax, p=part_boxes[jj].cpu().detach().numpy().reshape(-1),
                        color=color, rot=coord_rot)
            elif rep == 'geos':
                if part_geos[jj] is not None:
                    draw_geo(
                        ax=ax, p=part_geos[jj].cpu().detach().numpy().reshape(-1, 3),
                        color=color, rot=coord_rot)
            else:
