...
from utils import load_obj, sample_pc
...
anno_id = sys.argv[1]
in_dir = os.path.join('B:\\data_v0', anno_id)
...
def get_mesh(objs):
    v = []; f = []; vid = 0;
    for item in objs:
        vx,fx = load_obj(os.path.join(in_dir, 'objs', item+'.obj'))
        v.append(vx)
        f.append(fx)
        vid += vx.shape[0]
    v = np.vstack(v)
...
def get_pc(v, f, num_points=1000):
    pc= sample_pc(v, f, n_points=num_points)
    return pc
...
def render_mesh(vv, f, color=[0.216, 0.494, 0.722]):
    ...
    cmd = '\"C:\\Program Files\\Git\\usr\\bin\\bash.exe\" render.sh %s %s' % (tmp_obj, tmp_png)
    call(cmd, shell=True)
